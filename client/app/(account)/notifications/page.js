...
<div className="right h-full w-[750px] bg-white shadow-xl  p-6">
  <div className="mb-8 border-b border-gray-200 pb-4">
    <h2 className="text-3xl font-extrabold text-gradient text-black">
      🔔 Notifications
    </h2>
  </div>
  ...
  {notices?.length > 0 ? (
    <div className="space-y-6">
      {notices.slice().map((notice, index) => (
        <div
          key={index}
          className="bg-gradient-to-r from-gray-50 to-white border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] duration-200"
          onClick={() => {
  ...
  {!notice.read && (
    ...
    <div className="flex flex-col items-center justify-center mt-20 text-center">
      ...
      <Link
        href="/"
        className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-full text-sm sm:text-base transition shadow-md"
      >
        Start Shopping
      </Link>
    </div>
