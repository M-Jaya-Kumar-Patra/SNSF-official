...
import { FaCartPlus } from "react-icons/fa6";
  ...
  const Navbar = ({ fontClass, cartItems = [] }) => {
    ...
    return (
      <nav className=" sticky top-[-90px] z-[100]">
        <div className="px-8 py-3 flex items-center justify-between bg-gradient-to-l from-[#798ca8] via-[#334257] to-[#131e30] w-full">
          {/* Logo Section */}
              ...
              {cat.children?.length > 0 && (
                <div
                  className={`absolute top-full mt-4 ${index > catData.length - 3 ? 'right-0' : 'left-0'
                    } bg-white shadow-xl px-6 py-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100] overflow-auto scrollbar-hide`}
                  style={{
