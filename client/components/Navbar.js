  ...
  const Navbar = ({ fontClass, cartItems = [], minimized = false }) => {
    ...
    console.log("RRRRRRRRRRRRRRRRRRRRRRRRRRR, ", catData)
    ...
    return (
      <nav className={`fixed w-full top-0 z-[1000] bg-slate-900    ${isMd ? " h-[90px] ":"h-[80px]  sm:h-[90px] "}    ` }>
        ...
        <div className="  bg-slate-900 text-white ">
          {/* ================= TOP BAR ================= */}
        <div
          className={` max-w-[1600px] mx-auto px-2 sm:px-6 flex  justify-between items-center      ${isMd ? " h-[90px] ":"h-[80px]  sm:h-[90px] "}  `}
        > 
          {/* LOGO */}
...
{/* ===== MD TEXT NAVBAR (UI ONLY – Flipkart style) ===== */}
    ...
    <div className="max-w-[1200px] mx-auto px-4 relative">
      ...
      <div className="flex justify-center">
        <div
          ...
          "
        >
          {["Sofas", "Living Room", "Bedroom", "Dining", "Study & Office", "Home decor", "Accessories"].map((name) => (
            <button
              key={name}
              onClick={() => {
                const catId = catData?.find(c => c.name === name)?._id;
