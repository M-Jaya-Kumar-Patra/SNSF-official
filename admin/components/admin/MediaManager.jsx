...
export default function MediaManager({
  ...
  const getData = useCallback(async () => {
    const res = await fetchDataFromApi(listEndpoint);
    setItems(res?.data || []);
  }, [listEndpoint]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name.trim()) {
      return alert.alertBox({ type: "error", msg: nameRequiredMessage });
    }

    const res = await postData(createEndpoint, form, true);
  ...
  const confirmDelete = async () => {
    const res = await deleteData(getItemEndpoint(selectedId));
    if (!res.error) {
  ...
  const applyOrder = async () => {
    const itemsToUpdate = items.map((item, idx) => ({
      id: item._id,
      index: idx,
    }));
    await postData(reorderEndpoint, { [reorderBodyKey]: itemsToUpdate });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 px-6 py-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-3">
          ...
          >
            <AddIcon fontSize="small" /> {addButtonLabel}
          </button>
            ...
            {items
              ...
              .map((item, idx) => (
                ...
                >
                  ...
                  <td className="p-3 text-sm break-all">{item.url}</td>
                  ...
                  <td className="p-3">
                    <Switch
                      checked={item.status}
                      onChange={async () => {
                        const updated = items.map((currentItem) =>
                        ...
                        const res = await putData(getItemEndpoint(item._id), {
                          status: !item.status,
                        });
                        ...
                        if (res.error) {
                          alert.alertBox({
                            type: "error",
                            msg: res.message || updateFailedMessage,
                          });
                          setItems(items);
                        }
                      }}
