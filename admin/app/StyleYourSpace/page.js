...
const StyleYourSpace = () => {
  ...
  // ============================
  const getData = async () => {
    const res = await fetchDataFromApi("/api/style-your-space/getAll");
    setItems(res?.data || []);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name.trim())
      return alert.alertBox({ type: "error", msg: "Please enter room name" });

    const res = await postData("/api/style-your-space/create", form, true);
  ...
  const confirmDelete = async () => {
    const res = await deleteData(`/api/style-your-space/${selectedId}`);
    if (!res.error) {
  ...
  const applyOrder = async () => {
    const itemsToUpdate = items.map((i, idx) => ({ id: i._id, index: idx }));
    await postData("/api/style-your-space/reorder", { items: itemsToUpdate });
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Manage room inspirations shown on the website
          </p>
        </div>
            ...
            {items
              ...
              .map((item, idx) => (
                ...
                >
                  ...
                  <td className="p-3 break-all text-sm">{item.url}</td>
                  <td className="p-3 font-medium">{item.name}</td>
                  <td className="p-3 font-semibold">{item.index}</td>
                    ...
                    <Switch
                      ...
                      onChange={async () => {
                        ...
                        if (res.error) {
                          alert.alertBox({
                            type: "error",
                            msg: res.message || "Failed to update",
                          });
                          setItems(items);
                        }
                      }}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                ...
                <UploadBox
                  multiple
                  name="images"
                  url="/api/style-your-space/uploadImages"
