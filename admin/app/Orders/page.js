...
const Orders = () => {
    ...
    const handleChangeOrderStatus = (orderId, newStatus) => {
        console.log("cccccccccccccccccccccxxxxxxxxxx", orderId)
        postData(`/api/order/updateStatus?orderId=${orderId}`, { order_Status: newStatus }, true).then((res) => {
            if (!res.error) {
                    ...
                    {OrdersData?.map((order, index) => {
                        ...
                        return (
                            <tr key={index} className="border-b border-slate-300"  >
                                <td className="text-black px-4 py-2 align-top cursor-pointer" onClick={() => handleOrderDetails(order)}>{order?.orderId}</td>
                                ...
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>{order?.products?.length}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>{order?.paymentMethod || "N/A"}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>{order?.payment_status}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>{order?.paymentId}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>₹{order?.totalAmt}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>{order?.deliveryDate || "N/A"}</td>
                                <td className="text-black px-4 py-2 align-top" onClick={() => handleOrderDetails(order)}>
