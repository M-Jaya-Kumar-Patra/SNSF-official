...
const Slider = () => {
  ...
  const handleSubmitAddForm = async (e) => {
    ...
    if (!formFields.title || previews.length === 0) {
      alert.alertBox({
        type: "error",
    ...
    await postData("/api/homeSlider/create", formFields, true)
      .then((response) => {
        if (!response.error) {
          ...
          alert.alertBox({
            type: "error",
            msg: response.message || "Failed to create slide",
          });
        }
      })
      .catch((error) => {
  ...
  const handleDeleteSlide = async (e, slideId) => {
    ...
    try {
      ...
      alert.alertBox({
        type: "error",
        msg: "Network issue. Try later" || error.message,
      });
    }
  };
        ...
        <div className="w-full   px-6">
          <div className="flex justify-between items-center   ">
            ...
            <div className="flex gap-4">
              <button
                className="w-auto h-auto p-2 py-1 pr-3 rounded-md  bg-green-800 flex items-center gap-1 "
          ...
          <table className="w-full text-center border-collapse border text-black border-slate-200 rounded-md shadow-lg mt-4">
              ...
              {loading ? (
                ...
                slides.map((prd, index) => (
                  <tr key={index} className="border-b border-slate-300">
                    ...
                    <td className="font-semibold">{prd?.title}</td>
<td>{prd?.order}</td>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleCloseDeleteAlert} color="inherit">
            Cancel
          </Button>
          <Button
            onClick={(e) => {
              handleDeleteSlide(e, selectedProductId), setOpen(false);
            }}
            autoFocus
            color="error"
          >
        ...
        <div className="flex w-full h-full justify-center items-center bg-black bg-opacity-50 fixed top-0 left-0 z-50">
          <div className="w-[700px] h-[90%] bg-white rounded-md text-black py-3 px-6 overflow-auto scrollbar-hide">
            <form onSubmit={handleSubmitAddForm}>
              <div className="text-green-800  text-xl  border-b-2 border-slate-300 py-2 font-sans font-semibold flex gap-2 items-center">
                <ArrowBackIcon
              ...
              <div className="text-black my-3 font-sans font-semibold">
                Upload images
              </div>
              <div className="grid grid-cols-4 gap-4  mb-3">
                {Array.isArray(previews) &&
                  ...
                  previews.map((image, index) => (
                    <div
                      className="uploadBoxWrappper relative"
                      key={`${image}-${index}`}
                    >
                      <span
              ...
              <div className="relative w-full flex gap-2 right-0  justify-end mt-4">
                <button
                  className=" bg-white border  py-1  w-[90px] text-lg rounded-full text-red-600 border-red-600 hover:shadow-md inset font-medium"
                  onClick={() => setShowAddModal(false)}
                >
                  Cancel
                </button>
                <button
                  className=" bg-green-700  py-1  w-[90px] text-lg rounded-full hover:bg-green-600 hover:shadow-md hover:border-none text-white  font-medium"
                  type="submit"
                >
                  Save
                </button>
