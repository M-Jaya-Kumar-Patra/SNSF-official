...
import Link from "next/link"; import { Admin, Package, CreditCard, MapPin, Heart, RefreshCcw, Bell, LifeBuoy, LogOut } from "lucide-react";
...
const Account = () => {
    ...
    const router = useRouter();
     const { jwtAdmin, setJwtAdmin } = useAuth();
      const isNextAuth = session?.admin;
      const isJWT = jwtAdmin?.email;
    
    
      const avatar = isNextAuth
        ? session.admin.avatar || "/images/account.png"
        : jwtAdmin?.avatar || "/images/account.png";

        const fullName = isNextAuth
    ? session?.admin?.name || session?.admin?.admin?.name || ""
    : jwtAdmin?.name || "";
