...
const Subcategories = () => {
    ...
    const handleChangeProductCat = (event) => {
        ...
        setProductCat(selected);
        setCateges(prev => ({
            ...prev,
            parentIds: selected // << Note: changed to parentIds instead of parentId
        }));
    }
  ...
  setCateges(prev => ({
    ...
    const handleSubmit = async (e) => {
        ...
        await postData("/api/category/create", {
            name: categs.name.trim(),
            parentIds: productCat,
            images: previews
        })
    ...
    const handleSubmit2 = async (e) => {
        ...
        await postData("/api/category/create", {
            name: categs2.name.trim(),
            parentIds: [productCat2], // or allow multi-select for 3rd level too
            images: previews
        })
                                        ...
                                        <div className='left '>
                                            ...
                                            <Select
                                                ...
                                                renderValue={(selected) => selected.map(id => {
                                                    const found = catData.find(cat => cat._id === id);
                                            ...
                                            >
                                                {catData.map((item) => (
                                                    <MenuItem key={item._id} value={item._id}>
                                                        <Checkbox checked={productCat.indexOf(item._id) > -1} />
                                                        {item.name}
                                                    </MenuItem>
                                                ))}
                                            </Select>
